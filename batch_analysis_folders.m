% BATCH ANALYSIS THROUGH FOLDERS PIPELINE

function batch_analysis_folders(option,computer,varargin)
...
for p = 1 : length(session_names)
    ...
    for s = 1: length(folders)
        ...
        if any(ismember(option,3)) || strcmp(varargin,'all')
            ...
            sleep_stager('manual');
            %classify_sleep; %needs to be improved            
        end
...
%             % Scores shuffles post-hoc
%             shuffle_type= shuffle_scoring(shuffled_struct,[1 1 1 1],'whole');
%             save shuffled_tracks shuffle_type
...
%             shuffle_type1= shuffle_scoring(shuffled_struct1,[1 1 1 1],'segments');
%             shuffle_type2= shuffle_scoring(shuffled_struct2,[1 1 1 1],'segments');
%             save shuffled_tracks_segments shuffle_type1 shuffle_type2;
%             clear shuffle_type1 shuffle_type2
        ...
        if any(ismember(option,8)) || strcmp(varargin,'all')
            ...
            number_of_significant_replays(0.05,3,'linear',1); 
            number_of_significant_replays(0.05,3,'linear',2); 
            number_of_significant_replays(0.05,3,'path',1); 
            number_of_significant_replays(0.05,3,'path',2); 
            toc
        end
        ...
        if any(ismember(option,11)) || strcmp(varargin,'all')
            
            replay_control_short_exposures('short_exposure','last');
