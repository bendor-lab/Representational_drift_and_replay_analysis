function [freezing,quietWake,SWS,REM,movement] = detect_behavioural_states_masa(corticalLFP, hpcLFP, ...
                                                                                speed, speedTreshold, varargin)
...
% these are the periods we should exclude when detecting sleep and freezing
movement = SubtractIntervals([0 speed(end,1)],[noData;immobility]);
...
% Slow wave oscillation (0.5 to 4 Hz) or spindle ([9 17 ])
SR = 1/mean(diff(hpcLFP(:,1)));
...
spindleLFP = [hpcLFP(:,1) filtfilt(b_spindle,1,hpcLFP(:,2))];

% spindle_zscore  = zscore(abs(hilbert(LFP.spindle(nchannel,:))));
% spindleLFP = FilterLFP(corticalLFP,'passband',[9 17]);
% spindleLFP = bz_Filter(corticalLFP,'passband',[9 17]);

[~,bad,noisyIntervals] = CleanLFP(spindleLFP);
tLFP = spindleLFP(:,1);
badPeriods = tLFP(FindInterval(bad));
if ~isempty(noData) &  ~isempty(badPeriods)
...
if ~isempty(hpcLFP)
    ...
    REM = t(FindInterval(nanmean(q(4,:))>nanmean(q(1:3,:)))); % if power in the theta frequency band (7.5 Hz) is higher than low frequency power
else
...
% 28/09/2023 For now we ignore freezing
freezing = [];
%% Get quietWake
tic
