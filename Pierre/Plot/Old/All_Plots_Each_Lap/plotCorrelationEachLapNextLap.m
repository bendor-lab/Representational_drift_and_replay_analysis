...
% lap N of track 1 / 2 (physical) and next lap 
% NOTE : Don't work for 1 lap, need to do back, and then fourth

% ORIGINAL CODE (mainly the same) : plotCorrrelationEachLap
...
for file_ID = 1:length(listing_files)
    ...
    for lap = 1:(length(currentPFData) - 1) % We substract 1 because we compute the running corr
        
        currentPFVector = currentPFData(lap).place_fields;
        concurentPFVector = concurentPFData(lap + 1).place_fields;
        
        % We find the ID of shared good cells between the two
        currentGoodCells = currentPFData(lap).good_cells;
        
        concurrentGoodCells = concurentPFData(lap + 1).good_cells;
...
finalDataMean = finalData([finalData.condition] ~= "16" & [finalData.condition] ~= "1"); % We remove condition 1
...
unique_conditions = unique_conditions([2, 3, 4, 5, 1]);
