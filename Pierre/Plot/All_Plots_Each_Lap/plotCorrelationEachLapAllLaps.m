...
% lap N of track 1 / 2 (physical) and PF of all laps
...
% Path to get the already computed PF using all the laps
path_external_files = "X:\BendorLab\Drobo\Lab Members\Marta\Analysis\HIPP\";
...
for file_ID = 1:length(listing_files)
    ...
    if split_name(end) == "Track1" || split_name(end) == "Track3"
        ...
        % We get the mean place field for each cells 
        trackToCompareTo = 3;
    else
    ...
    % We get the concurrent_data by looking in extracted_place_fields (100 x_bins)
    % with respect to animal name and the condition
    
    animal_IO = split(filename, '_');
    animal_IO = animal_IO{2};
    path_to_ext_data_animal = path_external_files + animal_IO;
    all_files_animal = {dir(path_to_ext_data_animal).name};
    all_files_animal([1, 2]) = [];
    
    condition_OI = split(filename, '_');
    condition_OI = condition_OI{3};
    good_dir_condition = all_files_animal(contains(all_files_animal, condition_OI));
    good_dir_condition = good_dir_condition{1};
    
    concurentPFData = load(path_to_ext_data_animal + "\" + good_dir_condition + "\extracted_place_fields.mat");
    
    % Now that we have the concurent data, we take the mean of track 3 / 4 
    
    concurentPFData = concurentPFData.place_fields.track(trackToCompareTo);
