function place_fields = calculate_place_fields_LBL(x_bins_width, position, clusters, allclusters_waveform)

% INPUTS:
%   x_bin_width: enter width value (2 for fine resolution, 10 for bayesian
%   decoding).
%   position clusters allclusters_waveform -> modified variables to give
...
% Modified version for lap to lap computations

    parameters = list_of_parameters;

    % Extract the momemts where the animal is actually on the track
    for track_id=1:length(position.linear)
        % All the positions where the value is not NaNs
        track_indices = ~isnan(position.linear(track_id).linear);
        % gives the beggining and the end on each running session
    ...
    % Iterates thourgh all tracks
    for track_id = 1:length(position.linear)
        % Same thing as before, index the valid positions
        position_index = isnan(position.linear(track_id).linear);
        
        % Get the speed of the animal
        position_speed = abs(position.v_cm);
        position_speed(position_index) = NaN;  % make sure speed is NaN if position is NaN
        
        % Interpolates the position / speed at each spike time, given
        % position data
        position_during_spike = interp1(position.t,position.linear(track_id).linear,clusters.spike_times,'nearest');
        ...
        % x_hist represent the time spent at each x_bin (speed filtered)
        % List of conditions to be considered having been on the x_bin
        
        % ------ The position is timestamped after the start of running
        b1 = position.t>place_fields.track(track_id).time_window(1);
        % ------ The position is timestamped before the end of running
        b2 = position.t<place_fields.track(track_id).time_window(2);
        % ------ The speed is high enough to say that the animal was moving
        b3 = position_speed>parameters.speed_threshold_laps;
        % ------ The speed is low enough to say that it's not noise
        b4 = position_speed<parameters.speed_threshold_max;
        
        x_hist = time_bin_width.*histcounts(position.linear(track_id).linear(find(b1 & b2 & b3 & b4)),x_bin_edges);
        % Note : last argument --> Changed bin_centre to bin_edges
        
        % Save the values in the place_field object
        ...
        % On regarde POUR CHAQUE cellule, son nb de spike par bin (spike
        % hist) - conversion spike ID / cell ID par id_conversion
        for j = 1 : max(clusters.id_conversion(:,1))
            ...
            % PLACE FIELD CALCULATION for each cell (j)
            % Number of spike per bin / time per bin = frequency per bin !
            place_fields.track(track_id).raw{j} = place_fields.track(track_id).spike_hist{j}./x_hist;
            % Suppress all the NaNs if some
        ...
        % Calculate skagges information - bits of informations per seconds
