...
function scored_replay = replay_significance(scored_replay, shuffle_type)  %default [1 2 3 4]
% Extracts the replay event's p value by comparing the replay score with the distrbution of shuffled scores
% scored_replay is a variable of replay scores, shuffle_type is a variable containing the replay scores for each shuffle type
...
%extract replay scores for each replay event, across all tracks, across all methods
    for track = 1: num_tracks
        for event = 1 : size(scored_replay(track).replay_events,2)
            for s = 1 : num_shuffle_types % for each shuffle
