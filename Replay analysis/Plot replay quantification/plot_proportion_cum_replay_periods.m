function plot_proportion_cum_replay_periods(epoch)
    ...
    for i = 1 : length(protocols)
        ...
        for p = 1 : length(periods) %for each time period (sleep or run) within the protocol
            %if ~isempty(period_time(this_protocol_idx).(strcat(periods{p})).(strcat(epoch,'_cumulative_time')))
                ...
                for ii = 1 : length (this_protocol_idx)
                    [Num(ii,:),~] = histcounts([track_replay_events(this_protocol_idx(ii)).(sprintf('%s','T',num2str(track))).(strcat(periods{p},'_',epoch,'_cumulative_times'))],bin_edges); %events track 1
                    N(ii,:) = Num(ii,:)./binned_period_candidate_events.(sprintf('%s',epoch))(this_protocol_idx(ii)).(strcat(periods{p}));
                    N(ii,isnan(N(ii,:))) = 0;
                end
