function plot_awake_vs_post_replay(bayesian_control,rest_option,time_chunk_size,time_window)
% MH 2022
...
awake_rate_replay_T1(find(awake_rate_replay_T1 == 0)) = 0.5;
awake_rate_replay_T2(find(awake_rate_replay_T2 == 0)) = 0.5;
awake_rate_replay_RT1(find(awake_rate_replay_RT1 == 0)) = 0.5;
awake_rate_replay_RT2(find(awake_rate_replay_RT2 == 0)) = 0.5;

nexttile
hold on
awake = log2([awake_rate_replay_T1 awake_rate_replay_T2]);
...
plot(x,y_est,':','Color','black','LineWidth',3)
text(gca,.7,0.1,['p = ' num2str(pval,2)],'Units','Normalized','FontSize',12,'FontName','Arial');

mdl = fitlm(log2(awake_rate_replay_T1)',INTER_T1_rate_events');
[pval,~,~] = coefTest(mdl);
awake_rate_pval(1) = pval;
awake_rate_R2(1) = mdl.Rsquared.Adjusted;
x =[min(log2(awake_rate_replay_T1)) max(log2(awake_rate_replay_T1))];
b = mdl.Coefficients.Estimate';
y_est = polyval(fliplr(b),x);
...
nexttile
hold on
awake = log2([awake_rate_replay_RT1 awake_rate_replay_RT2]);
...
mdl = fitlm(log2(awake_rate_replay_RT1)',FINAL_RT1_rate_events');
[pval,~,~] = coefTest(mdl);
awake_rate_pval(3) = pval;
awake_rate_R2(3) = mdl.Rsquared.Adjusted;
x =[min(log2(awake_rate_replay_RT1)) max(log(awake_rate_replay_RT1))];
b = mdl.Coefficients.Estimate';
y_est = polyval(fliplr(b),x);
plot(x,y_est,':','Color',new_cls(1,:),'LineWidth',3)
...
mdl = fitlm(log2(awake_rate_replay_RT2)',log2(FINAL_RT2_rate_events)');
[pval,~,~] = coefTest(mdl);
awake_rate_pval(4) = pval;
awake_rate_R2(4) = mdl.Rsquared.Adjusted;
x =[min(log2(awake_rate_replay_RT2)) max(log2(awake_rate_replay_RT2))];
b = mdl.Coefficients.Estimate';
y_est = polyval(fliplr(b),x);
plot(x,y_est,':','Color',new_cls(20,:),'LineWidth',3)
...
%% Awake replay rate vs sleep replay rate (1 lap sessions removed) (First and Re-exposure separated)
% Awake replay rate vs sleep replay rate (First exposure)
nfig = figure('Color','w')
nfig.Position = [940 100 920 900];
...
nexttile
hold on
awake = [awake_rate_replay_T1(1:15) awake_rate_replay_T2(1:15)];
sleep = [INTER_T1_rate_events(1:15) INTER_T2_rate_events(1:15)];
new_cls = [repmat(PP.RUN1T1,15,1);repmat(PP.RUN1T2,15,1)];
arrayfun(@(x) scatter(awake(x),sleep(x),86,new_cls(x,:),'filled','o'),1:length(sleep))
...
mdl = fitlm(awake_rate_replay_T1(1:15)',INTER_T1_rate_events(1:15)');
[pval,~,~] = coefTest(mdl);
awake_rate_pval(1) = pval;
awake_rate_R2(1) = mdl.Rsquared.Adjusted;
x =[min(awake_rate_replay_T1(1:15)) max(awake_rate_replay_T1(1:15))];
b = mdl.Coefficients.Estimate';
y_est = polyval(fliplr(b),x);
plot(x,y_est,':','Color',new_cls(1,:),'LineWidth',3)
text1 = sprintf('T1 p = %.2d & R2 = %.3f',awake_rate_pval(1),awake_rate_R2(1))
...
mdl = fitlm(awake_rate_replay_T2(1:15)',INTER_T2_rate_events(1:15)');
[pval,F,~] = coefTest(mdl);
awake_rate_pval(2) = pval;
awake_rate_R2(2) = mdl.Rsquared.Adjusted;
x =[min(awake_rate_replay_T2(1:15)) max(awake_rate_replay_T2(1:15))];
b = mdl.Coefficients.Estimate';
y_est = polyval(fliplr(b),x);
plot(x,y_est,':','Color',new_cls(20,:),'LineWidth',3)
text2 = sprintf('T2 p = %.2d & R2 = %.3f',awake_rate_pval(2),awake_rate_R2(2));
...
set(gca,'FontSize',14)
ylim([0 0.09])
title(sprintf('Awake rate vs Sleep replay first exposure (1 lap sessions removed) (%s)',rest_option));
f=get(gca,'Children');
% Mind that order is reversed
legend([f(end),f(end-19)],text1,text2) %because f(1) and f(2) are lines
axis square
...
nexttile
hold on
awake = [awake_rate_replay_RT1(1:15) awake_rate_replay_RT2(1:15)];
sleep = [FINAL_RT1_rate_events(1:15) FINAL_RT2_rate_events(1:15)];
new_cls = [repmat(PP.RUN2T1,15,1);repmat(PP.RUN2T2,15,1)];
arrayfun(@(x) scatter(awake(x),sleep(x),86,new_cls(x,:),'filled','o'),1:length(sleep))
...
mdl = fitlm(awake_rate_replay_RT1(1:15)',FINAL_RT1_rate_events(1:15)');
[pval,~,~] = coefTest(mdl);
awake_rate_pval(3) = pval;
awake_rate_R2(3) = mdl.Rsquared.Adjusted;
x =[min(awake_rate_replay_RT1(1:15)) max(awake_rate_replay_RT1(1:15))];
b = mdl.Coefficients.Estimate';
y_est = polyval(fliplr(b),x);
plot(x,y_est,':','Color',new_cls(1,:),'LineWidth',3)
text1 = sprintf('T1 p = %.2d & R2 = %.3f',awake_rate_pval(3),awake_rate_R2(3))
...
mdl = fitlm(awake_rate_replay_RT2(1:15)',FINAL_RT2_rate_events(1:15)');
[pval,~,~] = coefTest(mdl);
awake_rate_pval(4) = pval;
awake_rate_R2(4) = mdl.Rsquared.Adjusted;
x =[min(awake_rate_replay_RT2(1:15)) max(awake_rate_replay_RT2(1:15))];
b = mdl.Coefficients.Estimate';
y_est = polyval(fliplr(b),x);
plot(x,y_est,':','Color',new_cls(20,:),'LineWidth',3)
text2 = sprintf('T2 p = %.2d & R2 = %.3f',awake_rate_pval(4),awake_rate_R2(4));
...
set(gca,'FontSize',14)
ylim([0 0.09])
title(sprintf('Awake rate vs Sleep replay re-exposure (1 lap session removed) (%s)',rest_option));
legend([f(end),f(end-19)],text1,text2) %because f(1) and f(2) are lines
axis square
...
%% Awake replay number vs Sleep replay rate (log2 scale) (First and second exposure separated)
nfig = figure('Color','w')
nfig.Position = [940 100 920 900];
awake_local_replay_T1(find(awake_local_replay_T1 == 0)) = 0.5;
awake_local_replay_T2(find(awake_local_replay_T2 == 0)) = 0.5;
awake_local_replay_RT1(find(awake_local_replay_RT1 == 0)) = 0.5;
awake_local_replay_RT2(find(awake_local_replay_RT2 == 0)) = 0.5;

nexttile
hold on
awake = log2([awake_local_replay_T1 awake_local_replay_T2]);
sleep = [INTER_T1_rate_events INTER_T2_rate_events];
new_cls = [repmat(PP.RUN1T1,19,1);repmat(PP.RUN1T2,19,1)];
arrayfun(@(x) scatter(awake(x),sleep(x),86,new_cls(x,:),'filled','o'),1:length(sleep))
...
mdl = fitlm(log2(awake_local_replay_T1)',INTER_T1_rate_events');
[pval,~,~] = coefTest(mdl);
awake_rate_pval(1) = pval;
awake_rate_R2(1) = mdl.Rsquared.Adjusted;
x =[min(log2(awake_local_replay_T1)) max(log2(awake_local_replay_T1))];
b = mdl.Coefficients.Estimate';
y_est = polyval(fliplr(b),x);
plot(x,y_est,':','Color',new_cls(1,:),'LineWidth',3)
text1 = sprintf('T1 p = %.2d & R2 = %.3f',awake_rate_pval(1),awake_rate_R2(1))
...
mdl = fitlm(log2(awake_local_replay_T2)',INTER_T2_rate_events');
[pval,F,~] = coefTest(mdl);
awake_rate_pval(2) = pval;
awake_rate_R2(2) = mdl.Rsquared.Adjusted;
x =[min(log2(awake_local_replay_T2)) max(log2(awake_local_replay_T2))];
b = mdl.Coefficients.Estimate';
y_est = polyval(fliplr(b),x);
plot(x,y_est,':','Color',new_cls(20,:),'LineWidth',3)
text2 = sprintf('T2 p = %.2d & R2 = %.3f',awake_rate_pval(2),awake_rate_R2(2));
% text(gca,.7,0.1,['Track 1 p = ' num2str(pval,2)],'Units','Normalized','FontSize',12,'FontName','Arial');
xlabel('Awake replay number (log2 scale)')
ylabel('Sleep replay rate')
set(gca,'FontSize',14)
ylim([0 0.09])
title(sprintf('Awake number vs Sleep replay first exposure (%s)',rest_option));
...
nexttile
hold on
awake = log2([awake_local_replay_RT1 awake_local_replay_RT2]);
sleep = [FINAL_RT1_rate_events FINAL_RT2_rate_events];
new_cls = [repmat(PP.RUN2T1,19,1);repmat(PP.RUN2T2,19,1)];
arrayfun(@(x) scatter(awake(x),sleep(x),86,new_cls(x,:),'filled','o'),1:length(sleep))
...
mdl = fitlm(log2(awake_local_replay_RT1)',FINAL_RT1_rate_events');
[pval,~,~] = coefTest(mdl);
awake_rate_pval(3) = pval;
awake_rate_R2(3) = mdl.Rsquared.Adjusted;
x =[min(log2(awake_local_replay_RT1)) max(log2(awake_local_replay_RT1))];
b = mdl.Coefficients.Estimate';
y_est = polyval(fliplr(b),x);
plot(x,y_est,':','Color',new_cls(1,:),'LineWidth',3)
text1 = sprintf('T1 p = %.2d & R2 = %.3f',awake_rate_pval(3),awake_rate_R2(3))
...
mdl = fitlm(log2(awake_local_replay_RT2)',FINAL_RT2_rate_events');
[pval,~,~] = coefTest(mdl);
awake_rate_pval(4) = pval;
awake_rate_R2(4) = mdl.Rsquared.Adjusted;
x =[min(log2(awake_local_replay_RT2)) max(log2(awake_local_replay_RT2))];
b = mdl.Coefficients.Estimate';
y_est = polyval(fliplr(b),x);
plot(x,y_est,':','Color',new_cls(20,:),'LineWidth',3)
text2 = sprintf('T2 p = %.2d & R2 = %.3f',awake_rate_pval(4),awake_rate_R2(4));
...
%% Awake replay number vs Sleep replay rate (1 lap sessions removed) (First and second exposure separated)
nfig = figure('Color','w')
nfig.Position = [940 100 920 900];

nexttile
hold on
awake = [awake_local_replay_T1(1:15) awake_local_replay_T2(1:15)];
sleep = [INTER_T1_rate_events(1:15) INTER_T2_rate_events(1:15)];
new_cls = [repmat(PP.RUN1T1,15,1);repmat(PP.RUN1T2,15,1)];
arrayfun(@(x) scatter(awake(x),sleep(x),86,new_cls(x,:),'filled','o'),1:length(sleep))
...
mdl = fitlm(awake_local_replay_T1(1:15)',INTER_T1_rate_events(1:15)');
[pval,~,~] = coefTest(mdl);
awake_number_pval(1) = pval;
awake_number_R2(1) = mdl.Rsquared.Adjusted;
x =[min(awake_local_replay_T1(1:15)) max(awake_local_replay_T1(1:15))];
b = mdl.Coefficients.Estimate';
y_est = polyval(fliplr(b),x);
plot(x,y_est,':','Color',new_cls(1,:),'LineWidth',3)
text1 = sprintf('T1 p = %.2d & R2 = %.3f',awake_number_pval(1),awake_number_R2(1))
...
mdl = fitlm(awake_local_replay_T2(1:15)',INTER_T2_rate_events(1:15)');
[pval,~,~] = coefTest(mdl);
awake_number_pval(2) = pval;
awake_number_R2(2) = mdl.Rsquared.Adjusted;
x =[min(awake_local_replay_T2(1:15)) max(awake_local_replay_T2(1:15))];
b = mdl.Coefficients.Estimate';
y_est = polyval(fliplr(b),x);
plot(x,y_est,':','Color',new_cls(20,:),'LineWidth',3)
text2 = sprintf('T2 p = %.2d & R2 = %.3f',awake_number_pval(2),awake_number_R2(2));
...
set(gca,'FontSize',14)
ylim([0 0.09])
title(sprintf('Awake number vs Sleep replay first exposure (1 lap sessions removed) (%s)',rest_option));
f=get(gca,'Children');
% Mind that order is reversed
legend([f(end),f(end-19)],text1,text2) %because f(1) and f(2) are lines
axis square
...
nexttile
hold on
awake = [awake_local_replay_RT1(1:15) awake_local_replay_RT2(1:15)];
sleep = [FINAL_RT1_rate_events(1:15) FINAL_RT2_rate_events(1:15)];
new_cls = [repmat(PP.RUN2T1,15,1);repmat(PP.RUN2T2,15,1)];
arrayfun(@(x) scatter(awake(x),sleep(x),86,new_cls(x,:),'filled','o'),1:length(sleep))
...
mdl = fitlm(awake_local_replay_RT1(1:15)',FINAL_RT1_rate_events(1:15)');
[pval,~,~] = coefTest(mdl);
awake_number_pval(3) = pval;
awake_number_R2(3) = mdl.Rsquared.Adjusted;
x =[min(awake_local_replay_RT1(1:15)) max(awake_local_replay_RT1(1:15))];
b = mdl.Coefficients.Estimate';
y_est = polyval(fliplr(b),x);
plot(x,y_est,':','Color',new_cls(1,:),'LineWidth',3)
text1 = sprintf('T1 p = %.2d & R2 = %.3f',awake_number_pval(3),awake_number_R2(3))
...
mdl = fitlm(awake_local_replay_RT2(1:15)',FINAL_RT2_rate_events(1:15)');
[pval,~,~] = coefTest(mdl);
awake_number_pval(4) = pval;
awake_number_R2(4) = mdl.Rsquared.Adjusted;
x =[min(awake_local_replay_RT2(1:15)) max(awake_local_replay_RT2(1:15))];
b = mdl.Coefficients.Estimate';
y_est = polyval(fliplr(b),x);
plot(x,y_est,':','Color',new_cls(20,:),'LineWidth',3)
text2 = sprintf('T2 p = %.2d & R2 = %.3f',awake_number_pval(4),awake_number_R2(4));
...
set(gca,'FontSize',14)
ylim([0 0.09])
title(sprintf('Awake number vs Sleep replay re-exposure (1 lap sessions removed) (%s)',rest_option));
legend([f(end),f(end-19)],text1,text2,'Location','northeast') %because f(1) and f(2) are lines
axis square
