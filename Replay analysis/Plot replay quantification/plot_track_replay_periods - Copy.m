...
for track = 1 : num_tracks
    ...
    for i = 1 : length(protocols)
        ...
        for p = 1 : length(periods) %for each time period (sleep or run) within the protocol
            ...
            %if ~isempty(period_time(this_protocol_idx).(strcat(periods{p})).(strcat(epoch,'_cumulative_time')))
                ...
                event_count(i).(strcat(periods{p})) = smooth(N./bins_with_active_period(1:end-1),5)/bin_width;
                event_count(i).(strcat(periods{p},'_cumsum')) = cumsum(N./bins_with_active_period(1:end-1))/bin_width;
    ...
    if length(periods) > 3 % more than 2 tracks
        ...
        for ii = 1 : length(T(track).event_count)
            ...
            plot(1:half_rats_thresh,T(track).event_count{1,ii}.INTER_post(1:half_rats_thresh),'Color',PP.T2(ii,:),'LineWidth',2)
            hold on
            plot(half_rats_thresh:length(T(track).event_count{1,ii}.INTER_post),T(track).event_count{1,ii}.INTER_post(half_rats_thresh:end),'Color',PP.T2(ii,:),'LineWidth',2,'LineStyle',':')
        end
        box off
    ...
    for ii = 1 : length(T(track).event_count)
        ...
        if ~isempty(half_rats_thresh)
            pl(ii) = plot(1:half_rats_thresh,T(track).event_count{1,ii}.FINAL_post(1:half_rats_thresh),'Color',PP.T2(ii,:),'LineWidth',2);
            hold on
            plot(half_rats_thresh:length(T(track).event_count{1,ii}.FINAL_post),T(track).event_count{1,ii}.FINAL_post(half_rats_thresh:end),'Color',PP.T2(ii,:),'LineWidth',2,'LineStyle',':')
        else
            pl(ii) = plot(1:length(T(track).event_count{1,ii}.FINAL_post),T(track).event_count{1,ii}.FINAL_post,'Color',PP.T2(ii,:),'LineWidth',2);
            hold on
        end
    end
    box off
