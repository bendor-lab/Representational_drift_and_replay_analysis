function raster_plot_replay_times(state)
...
load('extracted_time_periods_replay_excl.mat')
...
elseif strcmp(state,'awake') || strcmp(state,'ALL')
    ...
    periods = [{'PRE'},{'T1'},{'T2'},{'INTER_post'},{'T3'},{'T4'},{'FINAL_post'}];
%     periods = [{'PRE'},{'T1'},{'sleep_pot1'},{'T2'},{'INTER_post'},{'T3'},{'sleep_pot2'},{'T4'},{'FINAL_post'}];
    ...
    for j = 1 : length(track_replay_events)
        raster_session(j).all_norm_sig_event_times = [];
        start_indices = [];
        for p = 1 : length(periods)
            
            if p ~= 1
                start_time = ALLPeriods_time_end(p-1);
            else
                start_time = 0;
            end
            
            if track == 1 % Track 1 and Track 3 as Track 1
%                 if ismember(p,[1,6,7,8,9])
                if ismember(p,[5,6,7])
                ...
                if ismember(p,[2,3,5,6])
%                      if ismember(p,[2,4,6,8])
                    raster_session(j).all_norm_sig_event_times = [raster_session(j).all_norm_sig_event_times...
                        start_time + track_replay_events(j).(sprintf('%s','T',num2str(track),'_normalized')).(strcat(periods{p},'_norm_times'))];
                    
                    index = track_replay_events(j).(sprintf('%s','T',num2str(track))).(strcat(periods{p},'_index'));
                else
                    raster_session(j).all_norm_sig_event_times = [raster_session(j).all_norm_sig_event_times...
                        start_time + track_replay_events(j).(sprintf('%s','T',num2str(track),'_normalized')).(strcat(periods{p},'_norm_ALL_times'))];
                    
                    index = track_replay_events(j).(sprintf('%s','T',num2str(track))).(strcat(periods{p},'_merged_index'));
                end
                
                if ~isempty(index)
                    start_indices = [start_indices index(1)];
                    all_ids = [all_ids j*ones(1,length((index)))];
                end
                
            elseif track == 2
%                 if ismember(p,[1,6,7,8,9])
                if ismember(p,[5,6,7]) % Track 2 and Track 4 as Track 2
                    track_replay_events = track_replay_events_R.track_replay_events;
                else
                    track_replay_events = track_replay_events_F.track_replay_events;
                end
...
%                 if ismember(p,[2,4,6,8])
                ...
                if ~isempty(index)
                    start_indices = [start_indices index(1)];
                    all_ids = [all_ids j*ones(1,length((index)))];
%                      all_ids = [all_ids 1.25*j*ones(1,length((index)))];
%                     all_vals = [];
                end
            end
        end
...
elseif strcmp(state,'ALL') || strcmp(state,'awake')
    ...
    for i = 1 : 5 % 5 protocols\
        if i == 2
            cols = [cols; repmat(PP.T2(i,:),length([raster_session(count:count+2).all_norm_sig_event_times]),1)];
            count = count + 3;
        else
    ...
    raster_plot([raster_session(:).all_norm_sig_event_times]',all_ids,cols,0.75)
    hold on
...
%     
    annotation('textbox',[0.14 0.85 0.1 0.1],'String',strcat('PRE'),'EdgeColor', 'none','HorizontalAlignment', 'center','Fontsize',14);
...
%     annotation('textbox',[0.27 0.85 0.1 0.1],'String',strcat('REST 1'),'EdgeColor', 'none','HorizontalAlignment', 'center','Fontsize',14);
    annotation('textbox',[0.263 0.85 0.1 0.1],'String',strcat('T2'),'EdgeColor', 'none','HorizontalAlignment', 'center','Fontsize',14);
    annotation('textbox',[0.41 0.85 0.1 0.1],'String',strcat('POST1'),'EdgeColor', 'none','HorizontalAlignment', 'center','Fontsize',14);
    annotation('textbox',[0.58 0.85 0.1 0.1],'String',strcat('T3'),'EdgeColor', 'none','HorizontalAlignment', 'center','Fontsize',14);
%     annotation('textbox',[0.64 0.85 0.1 0.1],'String',strcat('REST 2'),'EdgeColor', 'none','HorizontalAlignment', 'center','Fontsize',14);
    annotation('textbox',[0.62 0.85 0.1 0.1],'String',strcat('T4'),'EdgeColor', 'none','HorizontalAlignment', 'center','Fontsize',14);
    annotation('textbox',[0.72 0.85 0.1 0.1],'String',strcat('POST2'),'EdgeColor', 'none','HorizontalAlignment', 'center','Fontsize',14);
...
set(gca,'YLim',[0 length(track_replay_events)+2],'ytick',[1.375:1:length(track_replay_events)+1],'yticklabel',names)
% xticks(0:200:24000)
% xticklabels()
...
elseif isnumeric(c) %colormap
    p = 1;
    for i = 1 : 3 :  length(x2)
        plot(x2(i:i+2),y2(i:i+2),'Color',c(p,:),'LineWidth',1);
        hold on
        p = p + 1;
    end
