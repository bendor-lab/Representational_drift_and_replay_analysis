function plot_track_replay_periods(epoch,bayesian_control)
    ...
    for p = 1 : length(periods) %for each time period (sleep or run) within the protocol
        ...
        for ii = 1 : t2
            if strcmp(epoch,'merged')
                mt = [mt; period_time(ii).(strcat(periods{p})).length];
            else
                if ~isempty(period_time(ii).(strcat(periods{p})).(strcat(epoch,'_cumulative_time')))
                    mt = [mt; max(period_time(ii).(strcat(periods{p})).(strcat(epoch,'_cumulative_time')),[],'all')];
                end
            end
        end
        ...
        for ii = 1 : length (t2)
            [N(ii,:),~] = histcounts([track_replay_events(ii).(sprintf('%s','T',num2str(track))).(strcat(periods{p},'_',epoch,'_cumulative_times'))],bin_edges); %events track 1
        end
        event_count(i).(strcat(periods{p},'_std')) = std(N/bin_width);
%         if size(N,1) > 1
%             N = sum(N/bin_width,1);
%         end
        ...
        bins_with_active_period = ones(1,length(bin_edges))*length(t2); % Set the count as if all periods had the same length
        for t = 1 : length(t2)
            if strcmp(epoch,'merged') & period_time(t).(strcat(periods{p})).length < longest_period_time % if this sessions is shorter than the longest period
                [~,idx] = min(abs(bin_edges - period_time(t).(strcat(periods{p})).length)); %find bin indx where session is not active anymore
                bins_with_active_period(idx+1:end) = bins_with_active_period(idx+1:end) - 1;
            elseif ~strcmp(epoch,'merged') & max(period_time(t).(strcat(periods{p})).(strcat(epoch,'_cumulative_time')),[],'all') < longest_period_time % if this sessions is shorter than the longest period
                [~,idx] = min(abs(bin_edges - max(period_time(t).(strcat(periods{p})).(strcat(epoch,'_cumulative_time')),[],'all'))); %find bin indx where session is not active anymore
                bins_with_active_period(idx+1:end) = bins_with_active_period(idx+1:end) - 1;
            end
        end
        
        % Sum events of all sessions of this protocol & divide events of each bin by number of active periods in that bin
        event_count(i).(strcat(periods{p},'_indivisual')) = N/bin_width;
        event_count(i).(strcat(periods{p})) = sum(N/bin_width,1)./bins_with_active_period(1:end-1);
%         event_count(i).(strcat(periods{p})) = smooth(N./bins_with_active_period(1:end-1),5);
        event_count(i).(strcat(periods{p},'_cumsum')) = cumsum(sum(N/bin_width,1)./bins_with_active_period(1:end-1));
    ...
    if length(periods) == 3 % 2 tracks only
        f1(track).Name = strcat('Sleep replay rate over time during FINAL sleep- Track',num2str(track),'-',epoch);
        subplot_idx = 1;
        sub_ix = [0 1 0 2];
    else
        f1(track).Name = strcat('Sleep replay rate over time during INTER and FINAL sleep- Track',num2str(track),'-',epoch);
        subplot_idx = 2;
        sub_ix = [1 2 3 4];
    end
    ...
    if length(periods) > 3 % more than 2 tracks
        ...
        for ii = 1 : length(T(track).event_count)
            ...
            x = 1:numel(T(track).event_count{1,ii}.INTER_post);
            shade1 = T(track).event_count{1,ii}.INTER_post + T(track).event_count{1,ii}.INTER_post_std;
            shade2 = T(track).event_count{1,ii}.INTER_post - T(track).event_count{1,ii}.INTER_post_std;
            x2 = [x,fliplr(x)];
            inBetween = [shade1,fliplr(shade2)];
            h=fill(x2,inBetween,PP.T1(ii,:));
            set(h,'facealpha',0.2,'LineStyle','none')
            hold on
            
            % Find at which time bin starts being information only for half
            % of the sessions
            ...
            plot(1:half_rats_thresh,T(track).event_count{1,ii}.INTER_post(1:half_rats_thresh),'Color',PP.T1(ii,:),'LineWidth',2)
            hold on
            plot(half_rats_thresh:length(T(track).event_count{1,ii}.INTER_post),T(track).event_count{1,ii}.INTER_post(half_rats_thresh:end),'Color',PP.T1(ii,:),'LineWidth',2,'LineStyle',':')
            
            %             replay_rate = T(track).event_count{i}.INTER_post_indivisual;
            %             replay_rate = reshape(replay_rate,1,size(replay_rate,1)*size(replay_rate,2));
            
            replay_rate = T(track).event_count{i}.INTER_post;
            mdl = fitlm(1:1:length(replay_rate),replay_rate');
            [pval,F_stat,~] = coefTest(mdl);
            R2 = mdl.Rsquared.Adjusted;
            x =[1 length(replay_rate)];
            b = mdl.Coefficients.Estimate';
            y_est = polyval(fliplr(b),x);
            plot(x,y_est,':','Color','r','LineWidth',3)
            f=get(gca,'Children');
...
%             text(gca,.7,0.1,sprintf('p = %.2d & R2 = %.3f',pval,R2),'Units','Normalized','FontName','Arial','FontSize',16);
            text(gca,.7,0.1,sprintf('p = %.2d & Beta = %.5f',pval,b(2)),'Units','Normalized','FontName','Arial','FontSize',16);
%             axis square
    ...
    for ii = 1 : length(T(track).event_count)
        ...
        % Add standard deviation as shade
        x = 1:numel(T(track).event_count{1,ii}.FINAL_post);
        shade1 = T(track).event_count{1,ii}.FINAL_post + T(track).event_count{1,ii}.FINAL_post_std;
        shade2 = T(track).event_count{1,ii}.FINAL_post - T(track).event_count{1,ii}.FINAL_post_std;
        x2 = [x,fliplr(x)];
        inBetween = [shade1,fliplr(shade2)];
        h=fill(x2,inBetween,PP.T1(ii,:));
        set(h,'facealpha',0.2,'LineStyle','none')
        ...
        if ~isempty(half_rats_thresh)
            pl(ii) = plot(1:half_rats_thresh,T(track).event_count{1,ii}.FINAL_post(1:half_rats_thresh),'Color',PP.T1(ii,:),'LineWidth',2);
            hold on
            plot(half_rats_thresh:length(T(track).event_count{1,ii}.FINAL_post),T(track).event_count{1,ii}.FINAL_post(half_rats_thresh:end),'Color',PP.T1(ii,:),'LineWidth',2,'LineStyle',':')
        else
            pl(ii) = plot(1:length(T(track).event_count{1,ii}.FINAL_post),T(track).event_count{1,ii}.FINAL_post,'Color',PP.T1(ii,:),'LineWidth',2);
            hold on
        end
        
        replay_rate = T(track).event_count{i}.FINAL_post;
        mdl = fitlm(1:1:length(replay_rate),replay_rate');
        [pval,F_stat,~] = coefTest(mdl);
        R2 = mdl.Rsquared.Adjusted;
        x =[1 length(replay_rate)];
