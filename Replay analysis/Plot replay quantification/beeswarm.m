...
if isfinite(p.Results.dot_size)
    ...
    for i=1:length(ux)
        ...
        switch lower(p.Results.overlay_style)
            ...
            case 'sd'
                line([1 1]*ux(i),nanmean(yorig(ic==i))+[-1 1]*nanstd(yorig(ic==i)),'Color',cmap(i,:),'LineWidth',2)
                line([ux(i)-2*rwid ux(i)+2*rwid],[1 1]*nanmean(yorig(ic==i)),'LineWidth',3,'Color',cmap(i,:))
            case 'ci'
