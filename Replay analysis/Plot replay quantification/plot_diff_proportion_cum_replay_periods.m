function plot_diff_proportion_cum_replay_periods(epoch,multievents,bayesian_control)
...
% INPUT: 
    ...
    % multievents: 1 or [], to select replay events that are sig for both exposures
...
for c = 1 : length(comparisons) % For each type of comparison
    ...
    f(c).Name = strcat('Cumulative proportion replay events diff between tracks -',num2str(cell2mat(comparisons(c))),'during all sleep periods - ',epoch);
            ...
            %if ~isempty(period_time(this_protocol_idx).(strcat(periods{p})).(strcat(epoch,'_cumulative_time'))) %if this period exists
                ...
                for idx = 1 : length(this_protocol_idx)
                    if length(cell2mat(comparisons(c))) == 2
                        [Num1,~] = histcounts([track_replay_events(this_protocol_idx(idx)).(sprintf('%s','T',num2str(track(1)))).(strcat(periods{p},'_',epoch,'_cumulative_times'))],bin_edges); %events track 1
                        [Num2,~] = histcounts([track_replay_events(this_protocol_idx(idx)).(sprintf('%s','T',num2str(track(2)))).(strcat(periods{p},'_',epoch,'_cumulative_times'))],bin_edges); %events track 2
                        if isempty(binned_period_candidate_events.(sprintf('%s',epoch))(this_protocol_idx(idx)).(strcat(periods{p})))
                            N1 = Num1./0;
                            N2 = Num2./0;
                        else
                            N1 = Num1./binned_period_candidate_events.(sprintf('%s',epoch))(this_protocol_idx(idx)).(strcat(periods{p}));
                            N2 = Num2./binned_period_candidate_events.(sprintf('%s',epoch))(this_protocol_idx(idx)).(strcat(periods{p}));
                        end
                        N1(isnan(N1)) = 0;
                        N2(isnan(N2)) = 0;
                        ...
                        [Num1,~] = histcounts([track_replay_events(this_protocol_idx(idx)).(sprintf('%s','T',num2str(track(1)))).(strcat(periods{p},'_',epoch,'_cumulative_times'))],bin_edges);
                        [Num2,~] = histcounts([track_replay_events(this_protocol_idx(idx)).(sprintf('%s','T',num2str(track(2)))).(strcat(periods{p},'_',epoch,'_cumulative_times'))],bin_edges);
                        [Num3,~] = histcounts([track_replay_events(this_protocol_idx(idx)).(sprintf('%s','T',num2str(track(3)))).(strcat(periods{p},'_',epoch,'_cumulative_times'))],bin_edges);
                        [Num4,~] = histcounts([track_replay_events(this_protocol_idx(idx)).(sprintf('%s','T',num2str(track(4)))).(strcat(periods{p},'_',epoch,'_cumulative_times'))],bin_edges);
                        N1 = Num1./binned_period_candidate_events.(sprintf('%s',epoch))(this_protocol_idx(idx)).(strcat(periods{p}));
                        N1(isnan(N1)) = 0;
                        N2 = Num2./binned_period_candidate_events.(sprintf('%s',epoch))(this_protocol_idx(idx)).(strcat(periods{p}));
                        N2(isnan(N2)) = 0;
                        N3 = Num3./binned_period_candidate_events.(sprintf('%s',epoch))(this_protocol_idx(idx)).(strcat(periods{p}));
                        N3(isnan(N3)) = 0;
                        N4 = Num4./binned_period_candidate_events.(sprintf('%s',epoch))(this_protocol_idx(idx)).(strcat(periods{p}));
                        N4(isnan(N4)) = 0;
                        this_period_events = [this_period_events; (N1+N2)-(N3+N4)];
                        total_num_events = [total_num_events; (N1+N2+N3+N4)]; 
                    end
                end
                clear N1 N2 N3 N4 Num1 Num2 Num3 Num4
    ...
    % PLOT
    f10(c) = figure('units','normalized','outerposition',[0 0 1 1]);
    if length(periods) == 3 % 2 tracks only
        f10(c).Name = strcat('Cumulative proportion replay events difference between tracks -',num2str(cell2mat(comparisons(c))),' during FINAL sleep - ',epoch);
        subplot_idx = 1;
        sub_ix = [0 1 0 2];
    else
        f10(c).Name = strcat('Cumulative proportion replay events difference between tracks -',num2str(cell2mat(comparisons(c))),' during INTER and FINAL sleep - ',epoch);
        subplot_idx = 2;
        sub_ix = [1 2 3 4];
    end
    ...
    line(ax3,[min(xlim) max(xlim)],[0 0],'Color','k','LineWidth',1,'LineStyle','--')  % to separate between tracks
    box off
    xlabel('Binned time (min)','FontSize',16); ylabel('Cumulative replay bias','FontSize',16)
...
% if ~exist([path '\' epoch '_diff_proportion_cum_replay.mat'])
%     save([path '\' epoch '_diff_proportion_cum_replay.mat'],'T')
% end
