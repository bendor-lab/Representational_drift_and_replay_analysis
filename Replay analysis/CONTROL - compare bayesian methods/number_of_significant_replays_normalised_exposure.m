...
%     varargin: if not empty, then is using replay control data for short exposures            
...
if isempty(p_value_threshold)
    ...
    p_value_threshold_seg= p_value_threshold/2; 
elseif length(p_value_threshold)==1
    p_value_threshold_seg= p_value_threshold/2; 
elseif length(p_value_threshold)==2
    p_value_threshold_seg= p_value_threshold(2);
    p_value_threshold= p_value_threshold(1);
end
...
for track = 1 :  number_of_tracks % for each track
    ...
    for event = 1 : number_of_events %for each event
        ...
        sig_event_info.significance(track,event) = max(p_values.WHOLE(track,event,:))<p_value_threshold | max(p_values.FIRST_HALF(track,event,:))<p_value_threshold_seg | max(p_values.SECOND_HALF(track,event,:))<p_value_threshold_seg; % indices - 0 or 1 for significant event
        ...
        for i = 1 : size(p_values.WHOLE,3) %checks if event is significant with a specific type of shuffle (for either WHOLE and/or event segments)
            sig_event_info.method_idx(track,event,i) = [(p_values.WHOLE(track,event,i))<p_value_threshold | (p_values.FIRST_HALF(track,event,i))<p_value_threshold_seg | (p_values.SECOND_HALF(track,event,i))<p_value_threshold_seg];
        end
        
        % Is the highest p value within the 3 shuffles less than the p value threshold (for the whole event and for segments)
        sig_event_info.segment_idx(track,event,:) = [max(p_values.WHOLE(track,event,:))<p_value_threshold  max(p_values.FIRST_HALF(track,event,:))<p_value_threshold_seg  max(p_values.SECOND_HALF(track,event,:))<p_value_threshold_seg];
...
for i = 1:length(multi_tracks_index)
    if (length(find(sig_event_info.replay_segment_index(:,multi_tracks_index(i))==1))==0 & ...
        ...
        multi_tracks_ids= [multi_tracks_ids {find(sig_event_info.replay_segment_index(:,multi_tracks_index(i)))}];
    end
end
...
significant_replay_events.multi_tracks_index(1:length(multi_tracks_index),1) = num2cell(replay_above_rippleThresh_index(multi_tracks_index));
significant_replay_events.multi_tracks_index(:,2) = multi_tracks_ids;
...
% For each multi-track event, decide based on max bayesian score (or p-value if spearman)
    c = 1;
    ...
    for event = 1 : number_of_events
        ...
        pvalue_bias_ONLY_SIG_TRACKS(1:number_of_tracks) = NaN;
        ...
        if min(type_of_event)==3   %only second half significant
            ...
            pvalue_bias_ONLY_SIG_TRACKS(tracks_that_are_significant) = sig_event_info.p_value(tracks_that_are_significant,event); %for spearman, get pvalues of sig tracks
        ...
        if rexposure_option == 2  % Allow events to be significant for both first and second exposure only 1/2 and 3/4 are considered multi events
            if ~strcmp(method,'spearman')
                ...
                first_exposure_pvalue_bias = pvalue_bias_ONLY_SIG_TRACKS(1:2);
                second_exposure_pvalue_bias = pvalue_bias_ONLY_SIG_TRACKS(3:4);
            end

        % elseif original track and rexposure are significant, pick the one with a higher bayesian sum, and convert its bayesian sum into the sum of both the first and second exposure bayesian    
        elseif rexposure_option == 1 
            ...
            if ~isempty(find(tracks_that_are_significant==1)) &  ~isempty(find(tracks_that_are_significant==3)) %if event is sig for T1 and T1-R
                ...
                elseif strcmp(method,'spearman')
                    significant_replay_events.track1_and_1R_significant(size(significant_replay_events.track1_and_1R_significant,1)+1,1:3) = [event; pvalue_bias_ONLY_SIG_TRACKS(1); pvalue_bias_ONLY_SIG_TRACKS(3)];
                    if pvalue_bias_ONLY_SIG_TRACKS(1) <= pvalue_bias_ONLY_SIG_TRACKS(3) %if p value is smaller for T1, set that track as sig
                        pvalue_bias_ONLY_SIG_TRACKS(3) = NaN; % change T3 to 0, as not significant anymore since T1 has been selected
                        sig_event_info.significance(3,event) = 0;
                    else %if pvalue is smaller for T3
                        pvalue_bias_ONLY_SIG_TRACKS(1) = NaN;
                        sig_event_info.significance(1,event) = 0;
                    end
            ...
            if ~isempty(find(tracks_that_are_significant==2)) &  ~isempty(find(tracks_that_are_significant==4)) %if event is sig for T2 and T2-R
                ...
                elseif strcmp(method,'spearman')
                    significant_replay_events.track2_and_2R_significant(size(significant_replay_events.track2_and_2R_significant,1)+1,1:3) = [event; pvalue_bias_ONLY_SIG_TRACKS(2); pvalue_bias_ONLY_SIG_TRACKS(4)];
                    if pvalue_bias_ONLY_SIG_TRACKS(2) <= pvalue_bias_ONLY_SIG_TRACKS(4) %if p value is smaller for T2, set that track as sig
                        pvalue_bias_ONLY_SIG_TRACKS(4) = NaN; % change T3 to 0, as not significant anymore since T2 has been selected
                        sig_event_info.significance(4,event) = 0;
                    else %if pvalue is smaller for T4
                        pvalue_bias_ONLY_SIG_TRACKS(2) = NaN;
                        sig_event_info.significance(2,event) = 0;
                    end
        ...
        if ~strcmp(method,'spearman')
            ...
            if isempty(rexposure_option) & max(bayesian_bias_ONLY_SIG_TRACKS) >= (1.2/length(tracks_that_are_significant))  %60% threshold for 2 sig tracks. 40% for 3 sig tracks,
                ...
                % If re-exposure wih no duplicates (event can only be sig in 1 track exposure)
            elseif rexposure_option==1 & max(bayesian_bias_ONLY_SIG_TRACKS) >= 1.2/2  %max of two tracks
                bayesian_bias_selection = floor(bayesian_bias_ONLY_SIG_TRACKS/max(bayesian_bias_ONLY_SIG_TRACKS)); %max value will be one, all other values will be zero
                
                % If re-exposure with duplicates (event can be sig in both track exposures)
            elseif rexposure_option==2 & (max(first_exposure_bayesian_bias)>= 1.2/2 | max(second_exposure_bayesian_bias)>= 1.2/2) %max of two tracks first exposure/ max two tracks second exposure
                ...
                % If bayesian bias not different enough between tracks - exclude multi-track event
            elseif length(find(multi_tracks_index==event))~=0
                ...
                % If the event is not significant for any track
            elseif sum(sig_event_info.significance(:,event)) == 0
                bayesian_bias_selection = zeros(size(bayesian_bias_ONLY_SIG_TRACKS));
                
                % If only one track is significant
            elseif length(find(multi_tracks_index==event))==0
                bayesian_bias_selection= 1;
                
            else  % to alert potential exceptions
                disp('ERROR')
                bayesian_bias_selection= 1;  %don't change anything if only one track is significant
            end
        ...
        elseif strcmp(method,'spearman') %if using spearman, select based on p-values
            
            % If no track re-exposures and there is still more than one track sig
            if isempty(rexposure_option) & length(isnan(pvalue_bias_ONLY_SIG_TRACKS)) > 1
                pvalue_bias_selection = floor(pvalue_bias_ONLY_SIG_TRACKS/min(pvalue_bias_ONLY_SIG_TRACKS)); %min pvalue will be 1
                pvalue_bias_selection(pvalue_bias_selection ~=1) = 0; % convert to 0 the rest of values
                
                if sum(pvalue_bias_selection) > 1 %if the multiple p-values were the same, then exclude this event
                    significant_replay_events.BAYESIAN_BIAS_excluded_events_index(c) = event; %events excluded because had same p-values
                    c = c+1;
                    pvalue_bias_selection = zeros(size(pvalue_bias_ONLY_SIG_TRACKS));
                end
                
                % If re-exposure wih no duplicates (event can only be sig in 1 track exposure)
            elseif rexposure_option==1 & length(isnan(pvalue_bias_ONLY_SIG_TRACKS)) > 1  %min of two tracks
                pvalue_bias_selection = floor(pvalue_bias_ONLY_SIG_TRACKS/min(pvalue_bias_ONLY_SIG_TRACKS)); %min pvalue will be 1
                pvalue_bias_selection(pvalue_bias_selection ~=1) = 0; % convert to 0 the rest of values
                ...
                % If re-exposure with duplicates (event can be sig in both track exposures)
            elseif rexposure_option==2 & length(isnan(first_exposure_pvalue_bias)) > 1 | length(isnan(second_exposure_pvalue_bias)) > 1
                pvalue_bias_selection = zeros(size(pvalue_bias_ONLY_SIG_TRACKS));
                pvalue_bias_selection(1:2) = floor(first_exposure_pvalue_bias/min(first_exposure_pvalue_bias)); %max value will be one, all other values will be zero
                pvalue_bias_selection(pvalue_bias_selection ~=1) = 0; % convert to 0 the rest of values
                second_selection = floor(second_exposure_pvalue_bias/max(second_exposure_pvalue_bias));
                second_selection(second_selection ~=1) = 0; % convert to 0 the rest of values
                pvalue_bias_selection(3:4) = second_selection;
                
                if sum(pvalue_bias_selection) > 2 %if the multiple p-values were the same, then exclude this event
                    significant_replay_events.BAYESIAN_BIAS_excluded_events_index(c) = event; %events excluded because had same p-values
                    c = c+1;
                    pvalue_bias_selection = zeros(size(pvalue_bias_ONLY_SIG_TRACKS));
                end
            ...
            elseif sum(sig_event_info.significance(:,event)) == 0
                ...
                % If only one track is significant
            elseif length(find(multi_tracks_index==event))==0
                pvalue_bias_selection = 1;
                
            else  % to alert potential exceptions
                disp('ERROR')
                pvalue_bias_selection = 1;  %don't change anything if only one track is significant
            end
            
            % For praticality, keep BEST_BAYESIAN name although it should be BEST P-VALUE
            sig_event_info.bayesian_bias_ONLY_SIG_TRACKS(:,event) = pvalue_bias_ONLY_SIG_TRACKS;
            sig_event_info.significance_BEST_BAYESIAN(:,event) = sig_event_info.significance(:,event).*pvalue_bias_selection'; %if multiple significant events, pick the one with the greater bayesian sum
            sig_event_info.bayesian_biasRAW(:,event) = bayesian_biasRAW; %keep for practicality
...
for track = 1 :  number_of_tracks
    ...
    for i = 1:length(significant_replay_events.track(track).index)
        if (significant_replay_events.track(track).event_segment_best_score(i)==1)  %if best score in WHOLE event
            ...
            significant_replay_events.track(track).time_edges{i} = decoded_replay_events(track).replay_events(replay_above_rippleThresh_index(significant_replay_events.track(track).index(i))).timebins_edges;
        ...
        elseif (significant_replay_events.track(track).event_segment_best_score(i)==2) %if best score in first half of event
            ...
            significant_replay_events.track(track).time_edges{i} = decoded_replay_events1(track).replay_events(replay_above_rippleThresh_index(significant_replay_events.track(track).index(i))).timebins_edges;
        ...
        elseif (significant_replay_events.track(track).event_segment_best_score(i)==3) %if best score in second half of event
            ...
            significant_replay_events.track(track).time_edges{i} = decoded_replay_events2(track).replay_events(replay_above_rippleThresh_index(significant_replay_events.track(track).index(i))).timebins_edges;
            ...
            significant_replay_events.track(track).time_edges{i} = NaN;
        end
    end
end
...
switch method
    case 'wcorr'
        if ~isempty(rexposure_option) && rexposure_option == 2
            save('significant_replay_events_wcorr_individual_exposures','significant_replay_events','-v7.3');
        else
            save('significant_replay_events_wcorr','significant_replay_events','-v7.3');
        end
    case 'spearman'
        if ~isempty(rexposure_option) && rexposure_option == 2
            save('significant_replay_events_spearman_individual_exposures','significant_replay_events','-v7.3');
        else
            save('significant_replay_events_spearman','significant_replay_events','-v7.3');
        end
    case 'linear'
        if ~isempty(rexposure_option) && rexposure_option == 2
            save('significant_replay_events_linear_individual_exposures','significant_replay_events','-v7.3');
        else
            save('significant_replay_events_linear','significant_replay_events','-v7.3');
        end
    case 'path'
        if ~isempty(rexposure_option) && rexposure_option == 2
            save('significant_replay_events_path_individual_exposures','significant_replay_events','-v7.3');
        else
            save('significant_replay_events_path','significant_replay_events','-v7.3');
        end
end
...
if strcmp(method,'path')   %path finding, pac-man method
    for track = 1 :  number_of_tracks % for each track
        for event = 1 : length(scored_replay(track).replay_events) %for each event
...
% also do for whole events
p_values.WHOLE(isnan(p_values.WHOLE)) = 1;
replay_scores.WHOLE(isnan(replay_scores.WHOLE)) = 0;
