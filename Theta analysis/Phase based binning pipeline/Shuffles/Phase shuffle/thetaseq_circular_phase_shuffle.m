% PHASE CIRCULAR SHUFFLE
% MH 2020
% Circular shuffle of starting phase, shifiting all columns together.
% Maintains relationship between phase and cell assembly spike probability, but not phase/position.
% INPUT:
...
function [phase_bin_shuffles,phase_shuffle] = thetaseq_circular_phase_shuffle(decoded_thetaSeq,num_shuffles,save_shuffles,save_option)
...
parfor jj = 1 : num_cores
    [out{jj}.shuffled_thetaseq,out2{jj}] = run_circular_phase_shuffles(loops,decoded_sequences,save_shuffles);
end
...
if save_shuffles == 1
    for d = 1 : length(future_fields)
        for t = 1 : num_tracks
            ...
            phase_bin_shuffles.(strcat(future_fields{d}))(t).mean_relative_position = vertcat(cellArray1{:});
            phase_bin_shuffles.(strcat(future_fields{d}))(t).mean_relative_position = phase_bin_shuffles.(strcat(future_fields{d}))(t).mean_relative_position(1:num_shuffles,:);
        end
    end
    if strcmp(save_option,'Y')
        save('Theta\phase_shuffles_theta_sequences', 'phase_bin_shuffles','-v7.3') % shuffled matrices
    end
end
...
for d = 1 : length(fields)
    for t = 1 : num_tracks
        for jj = 1 : num_cores
            ...
            if jj == 1
                phase_shuffle.(strcat(fields{d}))(t).quadrant_ratio(:,(jj:num_col)) = out{jj}.shuffled_thetaseq.(strcat(fields{d}))(t).quadrant_ratio;
                phase_shuffle.(strcat(fields{d}))(t).weighted_corr(:,(jj:num_col)) = out{jj}.shuffled_thetaseq.(strcat(fields{d}))(t).weighted_corr;
                %phase_shuffle.(strcat(fields{d}))(t).linear_score(:,(jj:num_col)) = out{jj}.shuffled_thetaseq.(strcat(fields{d}))(t).linear_score;
            else
                phase_shuffle.(strcat(fields{d}))(t).quadrant_ratio(:,((jj-1)*num_col+1:jj*num_col)) = out{jj}.shuffled_thetaseq.(strcat(fields{d}))(t).quadrant_ratio;
                phase_shuffle.(strcat(fields{d}))(t).weighted_corr(:,((jj-1)*num_col+1:jj*num_col)) = out{jj}.shuffled_thetaseq.(strcat(fields{d}))(t).weighted_corr;
                %phase_shuffle.(strcat(fields{d}))(t).linear_score(:,((jj-1)*num_col+1:jj*num_col)) = out{jj}.shuffled_thetaseq.(strcat(fields{d}))(t).linear_score;
            end
        end
    end
end
...
if strcmp(save_option,'Y')
    save('Theta\thetaseq_phase_shuffle','phase_shuffle','-v7.3') % scores
end
...
function [all_shuffles,matrix_shuffles] = run_circular_phase_shuffles(loops,decoded_sequences,save_shuffles)
...
for s = 1 : loops
    ...
    for d = 1 : length(fields)
        for t = 1 : num_tracks
            for event = 1 : length(shuffled_struct.(strcat(fields{d}))(t).theta_sequences)
                matrix_size = size(shuffled_struct.(strcat(fields{d}))(t).theta_sequences(event).decoded_position);
                shuffled_struct.(strcat(fields{d}))(t).theta_sequences(event).decoded_position = circshift(shuffled_struct.(strcat(fields{d}))(t).theta_sequences(event).decoded_position,ceil(rand*matrix_size(2)),2);
            end
        end
    end
    ...
    for d = 1 : length(fields) %for each direction
        for t = 1 : num_tracks % for each track
            ...
            centered_averaged_thetaSeq.(strcat(fields{d}))(t).weighted_corr = weighted_correlation(centered_averaged_thetaSeq.(strcat(fields{d}))(t).mean_relative_position);
        end
    end
...
%     end
    ...
    for d = 1 : length(fields)
        for t = 1 : num_tracks
            ...
            all_shuffles.(strcat(fields{d}))(t).quadrant_ratio(s) = centered_averaged_thetaSeq.(strcat(fields{d}))(t).quadrant_ratio;
            all_shuffles.(strcat(fields{d}))(t).weighted_corr(s) = centered_averaged_thetaSeq.(strcat(fields{d}))(t).weighted_corr;
            %all_shuffles.(strcat(fields{d}))(t).linear_score(s) = centered_averaged_thetaSeq.(strcat(fields{d}))(t).linear_score;
        end
    end
