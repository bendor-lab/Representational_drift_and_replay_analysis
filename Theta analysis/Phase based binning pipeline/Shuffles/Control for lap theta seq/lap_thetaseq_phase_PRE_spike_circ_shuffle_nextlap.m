...
function [PREspike_train_circ_shuffle_scores, PREspike_train_circular_shuffles] = lap_thetaseq_phase_PRE_spike_circ_shuffle_1(decoded_thetaSeq,directional_place_fields,theta_spike_count,track,lap_id,num_shuffles,save_shuffles,save_option)
...
for s = 1 : loops
    ...
    estimated_position.dir1 = bayesian_decoding_lap_thetaseq_1(directional_place_fields(track_id).dir1.Complete_Lap{1,lap_id},theta_spike_count,track_id);
    estimated_position.dir2 = bayesian_decoding_lap_thetaseq_1(directional_place_fields(track_id).dir2.Complete_Lap{1,lap_id},theta_spike_count,track_id);
    ...
    for d = 1 : length(future_fields) %for each direction
        central_cycle = centered_averaged_thetaSeq.(sprintf('%s',future_fields{d}))(track_id).mean_relative_position;
        centered_averaged_thetaSeq.(sprintf('%s',future_fields{d}))(track_id).weighted_corr = weighted_correlation(central_cycle);
    end

    % Adds the scoring results of each shuffle to the same structure
    for d = 1 : length(future_fields)
        all_shuffles.(sprintf('%s',future_fields{d})).quadrant_ratio(s) = centered_averaged_thetaSeq.(sprintf('%s',future_fields{d}))(track_id).quadrant_ratio;
        all_shuffles.(sprintf('%s',future_fields{d})).weighted_corr(s) = centered_averaged_thetaSeq.(sprintf('%s',future_fields{d}))(track_id).weighted_corr;
    end
