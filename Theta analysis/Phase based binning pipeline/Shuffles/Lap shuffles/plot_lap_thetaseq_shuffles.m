...
function plot_lap_thetaseq_shuffles(method)
...
for p = 1 : length(session_names)
    ...
    for s = 1: length(folders)
        ...
        for t = 1 : 2%length(phase_shuffle.unidirectional)
            ...
            hold on
            %histogram(phase_shuffle.unidirectional(t).(sprintf('%s',method)),'EdgeColor',[0.4 0.4 0.4],'LineWidth',2.5,'DisplayStyle','stairs')
            %histogram(PREspike_train_circ_shuffle_scores.unidirectional(t).(sprintf('%s',method)),'EdgeColor','b','LineWidth',2.5,'DisplayStyle','stairs')
            %histogram(position_shuffle.unidirectional(t).(sprintf('%s',method)),'EdgeColor',[0 0.2 0],'LineWidth',2.5,'DisplayStyle','stairs')
...
%             plot([abs(centered_averaged_thetaSeq.unidirectional(t).(sprintf('%s',method))) abs(centered_averaged_thetaSeq.unidirectional(t).(sprintf('%s',method)))],...
