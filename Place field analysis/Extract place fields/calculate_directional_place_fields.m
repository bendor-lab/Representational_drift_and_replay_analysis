...
% OUTPUT: place field structure for each direction


function place_fields = calculate_directional_place_fields(x_bins_width)
 ...
 for dir = 1 : 2 % for each direction
    ...
    %find mean rate spikes on all tracks / time on all tracks (for identifying putative interneurons later in the code)
    all_spikes = [];
    for track_id = 1:length(position.linear)
    ...
    for track_id = 1:length(position.linear)
        ...
        for j = 1 : max(clusters.id_conversion(:,1))
            ...
            place_fields.track(track_id).spike_hist{j} = histcounts(position_during_spike(find(directional_clusters(track_id).(strcat('spikes_dir',num2str(dir)))(:,1)==j & ...
   ...
   % putative pyramidal cells classification:  pyramidal cells that pass the 'Pyramidal type' threshold (but not need to be place cells)
    ...
    directional_place_fields(dir).place_fields = place_fields;
