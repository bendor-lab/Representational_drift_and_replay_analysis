function shuffle_population_vector_analysis(bayesian_option,num_shuffles,save_option)
...
% between same track exposures and between different tracks). There are two type of shuffles: 
    % cellID shuffle of the ratemaps to simulate global remapping
    % Multiply ratemap matrix by a vector ranging from 0.1 to 1.5 to simulate rate remapping
% INPUT:
...
for p = 1 : length(session_names)
        ...
        if exist(strcat(pwd,'\extracted_place_fields.mat'),'file')
            ...
            for sh = 1 : num_shuffles
                ...
                for tt = 1 : length(track)
                    ...
                    rate_change = 0.1 + (1.5-0.1) .* rand(size(track(tt).norm_ratemaps,1),1); %create a vector with random values from 0.1 to 1.5
            ...
            %save
            protocol_shuffle(p).session(s).shuffled_rateRemap_PPvector = shuffled_rateRemap_PPvector.population_vector;
            protocol_shuffle(p).session(s).shuffled_rateRemap_PPvector_pval = shuffled_rateRemap_PPvector.pval;
            ...
            %save
            protocol_shuffle(p).session(s).shuffled_rateRemap_PPvector_SECTION = shuffled_rateRemap_PPvector.section_population_vector;
            protocol_shuffle(p).session(s).shuffled_rateRemap_PPvector_pval_SECTION = shuffled_rateRemap_PPvector.section_ppvector_pval;
    ...
    for jj = 1 : length(protocol_shuffle(p).session)
        ...
        all_PPvectors_RR = [all_PPvectors_RR; protocol_shuffle(p).session(jj).shuffled_rateRemap_PPvector];
        all_section_PPvectors_RR = [all_section_PPvectors_RR; protocol_shuffle(p).session(jj).shuffled_rateRemap_PPvector_SECTION];
    end
    ...
    protocol_shuffle(p).all_PPvectors_rateRemap = all_PPvectors_RR;
    protocol_shuffle(p).all_section_PPvectors_rateRemap = all_section_PPvectors_RR;
...
if strcmp(save_option,'Y')
    save(sprintf('%s',save_path,'\shuffle_population_vector_data_bayesian.mat'),'protocol_shuffle','-v7.3');
end
