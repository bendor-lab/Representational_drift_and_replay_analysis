function firing_rate_population_vector_analysis(computer,bayesian_option,save_option)
% Marta Huelin_February 2020
% Computes a variation of population vector analysis between matrices for the same type of comparisons used in the population_vector_analysis.m code (meaning,
% between same track exposures and between different tracks). Calculates the firing rate distance between cells at equivalent position bins.
% Should measure the how much the firing rate varies between tracks being compared.
% INPUT:
...
for p = 1 : length(session_names)
    ...
    protocol_rate(p).session_ID = cell2mat(session_names(p));
    ...
    for s = 1: length(folders) % where 's' is a recorded session
        ...
        if exist(strcat(pwd,'\extracted_place_fields.mat'),'file')
            ...
            FR_PPvector.population_vector =[];
            curr_size = size(FR_PPvector.population_vector,1);
            for i = 1 : length(comparisons)
                comp = cell2mat(comparisons(i));
                for j = 1 : size(track(1).norm_ratemaps,2)
                    % substract each FR for cell in each position bin, raise to the power of 2, sum and square root
                    sqrt_diff = sqrt(sum((track(comp(1)).norm_ratemaps(:,j)-track(comp(2)).norm_ratemaps(:,j)).^2));
                    FR_PPvector.population_vector(curr_size+j,i) = sqrt_diff; %each column is a comparison, each row a position bin
                end
            end
            
            
            %save
            protocol_rate(p).session(s).firing_rate_PPvector = FR_PPvector.population_vector;
            protocol_rate(p).session(s).session_ID = folders(s);
            ...
            % Run Kruskal-Wallis
            sig_diff_idx = [];
            [pv,tbl,stats1]=kruskalwallis(FR_PPvector.population_vector,[],'off');
            protocol_rate(p).session(s).KW_pv = pv;
            protocol_rate(p).session(s).KW_table = tbl;
            if pv < 0.05
                [c,~,~,~] = multcompare(stats1,'ctype','dunn-sidak','Display','off'); % if anova pval is < 0.05, run multiple comparisons
                protocol_rate(p).session(s).multiple_comparisons = c;
            ...
            hold on
            for i = 1 : length(comparisons)
                hh = cdfplot(FR_PPvector.population_vector(:,i));
            ...
            [fighandle,Legendhandle,~,~,~] = violin(FR_PPvector.population_vector,'medc',[0.3 0.3 0.3]);
            ...
            if ~isempty(sig_diff_idx)
                maxy = max(ylim);
                miny = min(ylim);
                ylim([miny,maxy+(length(sig_diff_idx)*0.43)])
                
                % Add sig bars
                for ii = 1 : length(sig_diff_idx)
                    dist = maxy+0.1 + (0.43*(ii-1));
                    dist_s = dist + 0.13;
                    hold on
                    plot([c(sig_diff_idx(ii),1) c(sig_diff_idx(ii),2)], [dist dist], '-k', 'LineWidth',1)
                    plot([(c(sig_diff_idx(ii),1)+c(sig_diff_idx(ii),2))/2 (c(sig_diff_idx(ii),1)+c(sig_diff_idx(ii),2))/2], [dist_s dist_s], '*k','MarkerSize',2)
                end
            end
            ...
            lap_start = 1;
            lap_end = str2num(protocol_rate(p).session_ID(end)); %number of laps of this protocol
            ...
            % Calculate cell population vector for each track comparison by correlating each position bin between tracks
            FR_PPvector.section_population_vector =[];
            % Calculate cell population vector for each track comparison by correlating each position bin between tracks
            curr_size = size(FR_PPvector.section_population_vector,1);
            for i = 1 : length(comparisons)-3 %only run first 3 comparisons
                comp = cell2mat(comparisons(i));
                for j = 1 : size(track_section(1).norm_ratemaps,2)
                    % Create shuffled matrix for rate remapping: substract each FR for cell in each position bin, raise to the power of 2, sum and square root
                    sqrt_diff = sqrt(sum((track(comp(1)).norm_ratemaps(:,j)-track(comp(2)).norm_ratemaps(:,j)).^2));
                    FR_PPvector.section_population_vector(curr_size+j,i) = sqrt_diff; %each column is a comparison, each row a position bin
                end
            end
            
            % Run Kruskal-Wallis
            sig_diff_idx = [];
            [pv1,tbl2,stats2] = kruskalwallis(FR_PPvector.section_population_vector,[],'off');
            protocol_rate(p).session(s).section_KW_pv = pv1;
            protocol_rate(p).session(s).section_KW_table = tbl2;
            if pv1 < 0.05
                [s_c,~,~,~] = multcompare(stats2,'ctype','dunn-sidak','Display','off'); % if anova pval is < 0.05, run multiple comparisons
                protocol_rate(p).session(s).section_multiple_comparisons = s_c;
                sig_diff_idx = find(s_c(:,6)<0.05);
            end
            
            %save
            protocol_rate(p).session(s).firing_rate_PPvector_SECTION = FR_PPvector.section_population_vector;
            ...
            for i = 1 : length(comparisons)-3
                hh = cdfplot(FR_PPvector.section_population_vector(:,i));
                hh.Color = PP.comp(p).colorT(i,:);
                hh.LineWidth = 2;
            end
            xlabel('PV correlation','Fontsize',16); ylabel('Cumulative frequency','Fontsize',16)
            ...
            [fighandle,Legendhandle,~,~,~] = violin(FR_PPvector.section_population_vector,'medc',[0.3 0.3 0.3]);
            for i = 1 : length(fighandle)
                fighandle(i).FaceColor = PP.comp(p).colorT(i,:);
                fighandle(i).EdgeColor = [0.2 0.2 0.2];
                fighandle(i).LineWidth = 1;
            ...
            xticks([1:1:size(FR_PPvector.section_population_vector,2)])
            ...
            if ~isempty(sig_diff_idx)
                ...
                for ii = 1 : length(sig_diff_idx)
                    ...
                    dist_s = dist + 0.13;
                    hold on
                    plot([s_c(sig_diff_idx(ii),1) s_c(sig_diff_idx(ii),2)], [dist dist], '-k', 'LineWidth',1)
                    plot([(s_c(sig_diff_idx(ii),1)+s_c(sig_diff_idx(ii),2))/2 (s_c(sig_diff_idx(ii),1)+s_c(sig_diff_idx(ii),2))/2], [dist_s dist_s],'*k','MarkerSize',2)
                end
            end
    ...
    for jj = 1 : length(protocol_rate(p).session)
        all_PPvectors_RR = [all_PPvectors_RR; protocol_rate(p).session(jj).firing_rate_PPvector];
        all_section_PPvectors_RR = [all_section_PPvectors_RR; protocol_rate(p).session(jj).firing_rate_PPvector_SECTION];
    end
    protocol_rate(p).all_PPvectors_Firing_rate = all_PPvectors_RR;
    protocol_rate(p).all_section_PPvectors_Firing_rate = all_section_PPvectors_RR;
    
    % Run Kruskal-Wallis
    all_sig_diff_idx = []; sig_diff_idx= [];
    [pv3,tbl3,stats3] = kruskalwallis(all_PPvectors_RR,[],'off');
    [pv4,tbl4,stats4] = kruskalwallis(all_section_PPvectors_RR,[],'off');
    protocol_rate(p).KW_pv = pv3;
    protocol_rate(p).KW_table = tbl3;
    protocol_rate(p).section_KW_pv = pv4;
    protocol_rate(p).section_KW_table = tbl4;
    if pv3 < 0.05
        [all_c,~,~,~] = multcompare(stats3,'ctype','dunn-sidak','Display','off'); % if anova pval is < 0.05, run multiple comparisons
        protocol_rate(p).all_multiple_comparisons = all_c;
    ...
    if pv4 < 0.05
        ...
        protocol_rate(p).all_section_multiple_comparisons = all_s_c;
    ...
    ax(2) = axes('Position',cell2mat(coord(2)));
        [fighandle,Legendhandle,~,~,~] = violin(all_PPvectors_RR,'medc',[0.3 0.3 0.3]);
        for i = 1 : length(fighandle)
            fighandle(i).FaceColor =  PP.comp(p).colorT(i,:);
            fighandle(i).EdgeColor = [0.2 0.2 0.2];
            fighandle(i).LineWidth = 1;
        ...
        xticks([1:1:size(all_PPvectors_RR,2)])
    ...
    ax(3) = axes('Position',cell2mat(coord(3)));
        hold on
        for i = 1 : length(comparisons)-3
            hh = cdfplot(all_section_PPvectors_RR(:,i));
            hh.Color =  PP.comp(p).colorT(i,:);
            hh.LineWidth = 2;
        end
        xlabel('PV correlation','Fontsize',16); ylabel('Cumulative frequency','Fontsize',16)
    ...
    ax(4) = axes('Position',cell2mat(coord(4)));
        [fighandle1,Legendhandle,~,~,~] = violin(all_section_PPvectors_RR,'medc',[0.3 0.3 0.3]);
        ...
        xticks([1:1:size(all_section_PPvectors_RR,2)])
        xticklabels({'T1 vs T1-R','T2 vs T2-R','T2 vs T1'})
        a = get(gca,'XTickLabel');
        set(gca,'XTickLabel',a,'fontsize',16,'FontWeight','bold')
        Legendhandle.Visible = 'off';
        ...
        if ~isempty(sig_diff_idx)
            ...
            for ii = 1 : length(sig_diff_idx)
                ...
                dist_s = dist + 0.13;
                hold on
                plot([all_s_c(sig_diff_idx(ii),1) all_s_c(sig_diff_idx(ii),2)], [dist dist], '-k', 'LineWidth',1)
                plot([(all_s_c(sig_diff_idx(ii),1)+all_s_c(sig_diff_idx(ii),2))/2 (all_s_c(sig_diff_idx(ii),1)+all_s_c(sig_diff_idx(ii),2))/2], [dist_s dist_s],'*k','MarkerSize',2)
            end
        end
...
T1=[]; all_protocol_sections = [];
for jj = 1 : length(protocol_rate)
    T1 = [T1; protocol_rate(jj).all_PPvectors_Firing_rate(:,1)]; % concatenate all 16runs together
    T2(:,jj) =  protocol_rate(jj).all_PPvectors_Firing_rate(:,2);
    all_protocol_sections = [all_protocol_sections, protocol_rate(jj).all_section_PPvectors_Firing_rate(:,2:3)];
end
...
protocol_firing_rate = protocol_rate;
save_path = 'X:\BendorLab\Drobo\Lab Members\Marta\Analysis\HIPP\Chapter 1\Population_vector_analysis\';
cd(save_path)
if strcmp(save_option,'Y')
    if bayesian_option == 1
        save(sprintf('%s',save_path,'\firing_rate_population_vector_data_bayesian.mat'),'protocol_firing_rate','-v7.3');
    else
        save(sprintf('%s',save_path,'\firing_rate_population_vector_data.mat'),'protocol_firing_rate','-v7.3');
    end
end
