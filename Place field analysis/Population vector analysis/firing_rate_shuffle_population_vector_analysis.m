function firing_rate_shuffle_population_vector_analysis(computer,bayesian_option,num_shuffles,save_option)
% Marta Huelin_February 2020
% Computes a variation of population vector analysis between shuffled matrices for the same type of comparisons used in the population_vector_analysis.m code (meaning,
    ...
    for s = 1: length(folders) % where 's' is a recorded session
        ...
        if exist(strcat(pwd,'\extracted_place_fields.mat'),'file')
            ...
            for sh = 1 : num_shuffles
                shuffled_rate_remap_matrix= [];
                % Create shuffled matrix for rate remapping: multiply each ratemap with a random picked FR from the real distribution
                for tt = 1 : length(track)
                    rate_change = good_peakFR(tt,randperm(length(good_peakFR))); % pick a peakFR from the real distribution
                    shuffled_rate_remap_matrix(tt).norm_ratemaps  = track(tt).norm_ratemaps.*rate_change'; % multiply ratemaps by the correspondin random num
                    ...
                    shuffled_rate_remap_matrix(tt).norm_ratemaps(shuffled_rate_remap_matrix(tt).norm_ratemaps> max_FR) = max_FR + 0.01; %small change to make sure it doesn't have the same value than the real data
                end
                ...
                for i = 1 : length(comparisons)
                    ...
                    for j = 1 : size(track(1).norm_ratemaps,2)
                        % substract each FR for cell in each position bin, raise to the power of 2, sum and square root
                         sqrt_diff = sqrt(sum((track(comp(1)).norm_ratemaps(:,j)-shuffled_rate_remap_matrix(comp(2)).norm_ratemaps(:,j)).^2));
                         shuffled_rateRemap_PPvector.population_vector(curr_size+j,i) = sqrt_diff; %each column is a comparison, each row a position bin
                    end
                end
            end
            ...
            for sh = 1 : num_shuffles
                ...
                for tt = 1 : length(track_section)
                    rate_change = section_peakFT(tt,randperm(length(section_peakFT))); % pick a peakFR from the real distribution
                    shuffled_rate_remap_matrix(tt).norm_ratemaps  = track(tt).norm_ratemaps.*rate_change'; % multiply ratemaps by the correspondin random num
                    % find max peak FR in the real data matrix
                    max_FR = max(max(track(tt).norm_ratemaps));
                    % and make sure that the new matrix doesn't have values higher than this one (for FR credibility)
                ...
                for i = 1 : length(comparisons)-3 %only run first 3 comparisons
                    ...
                    for j = 1 : size(track_section(1).norm_ratemaps,2)
                        ...
                        sqrt_diff = sqrt(sum((track(comp(1)).norm_ratemaps(:,j)-shuffled_rate_remap_matrix(comp(2)).norm_ratemaps(:,j)).^2));
                        shuffled_rateRemap_PPvector.section_population_vector(curr_size+j,i) = sqrt_diff; %each column is a comparison, each row a position bin
                    end
                end
            end
...
protocol_firing_rate_shuffle = protocol_rate_shuffle;
save_path = 'X:\BendorLab\Drobo\Lab Members\Marta\Analysis\HIPP\Chapter 1\Population_vector_analysis\';
cd(save_path)
if strcmp(save_option,'Y')
    if bayesian_option == 1
        save(sprintf('%s',save_path,'\firing_rate_shuffle_population_vector_data_bayesian.mat'),'protocol_firing_rate_shuffle','-v7.3');
    else
        save(sprintf('%s',save_path,'\firing_rate_shuffle_population_vector_data.mat'),'protocol_firing_rate_shuffle','-v7.3');
    end
end
