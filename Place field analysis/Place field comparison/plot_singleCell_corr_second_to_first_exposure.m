...
% between the laps 13 to 16 from the re-exposure and each lap in the first exposure.
% INPUT:
...
function session_data = plot_singleCell_corr_second_to_first_exposure(parameter,save_option)
...
if exist(strcat('plotting_data_second_to_first_exposure_',parameter,'.mat') ,'file')
    load(strcat('plotting_data_second_to_first_exposure_',parameter,'.mat'))
else
...
for t = 1 : length(session_data(1).track_data) % 2 comparisons
    ...
    for p = 1 : length(protocol)
        ...
        if length(cell_stability_quantification) ~= 4
            p1= bar(cell_stability_quantification);%plot first 20 rows
        ...
        else %if plotting for 16x1 - only 4 values
            for ii = 1: length(cell_stability_quantification)
                hold on
                 p1= bar(ii,cell_stability_quantification(ii));%plot first 20 rows
                 p1(1).FaceColor = col(5-ii,:); p1(1).EdgeColor = col(5-ii,:); %low stability
            end    
        end
        ...
        xticks(1:1:size(modified_parameters_matrix(sorted_centreMas_indx,:),2))
        axis([0.5 size(modified_parameters_matrix(sorted_centreMas_indx,:),2)+0.5 1 size(modified_parameters_matrix(sorted_centreMas_indx,:),1)])
        ...
        if p >= 1 && t == 2
            ...
            filename = strcat('SingleCell_',parameter,'_T2_from_RE-T2_',num2str(p),'Laps');
            f.Name = filename;
        ...
        elseif t == 1
            ...
            filename = strcat('SingleCell_',parameter,'_T1_from_RE-T1');
            f.Name = filename;
        end
        ...
        if ~strcmp(filename,strcat('SingleCell_',parameter,'_T4_from_T2_1Lap'))
            ...
            hold on
            p2 = plot(cell_stability_percentage(:,1),'LineWidth',3,'Color',col(4,:));
            plot(cell_stability_percentage(:,1),'o','MarkerFaceColor',col(4,:),'MarkerEdgeColor',col(4,:),'MarkerSize',4);
            p3 = plot(cell_stability_percentage(:,2),'LineWidth',3,'Color',col(3,:));
            plot(cell_stability_percentage(:,2),'o','MarkerFaceColor',col(3,:),'MarkerEdgeColor',col(3,:),'MarkerSize',4);
            p4 = plot(cell_stability_percentage(:,3),'LineWidth',3,'Color',col(2,:));
            plot(cell_stability_percentage(:,3),'o','MarkerFaceColor',col(2,:),'MarkerEdgeColor',col(2,:),'MarkerSize',4);
            p5 = plot(cell_stability_percentage(:,4),'LineWidth',3,'Color',col(1,:));
            plot(cell_stability_percentage(:,4),'o','MarkerFaceColor',col(1,:),'MarkerEdgeColor',col(1,:),'MarkerSize',4);
            xlabel('Laps'); ylabel('% place cells')
            xticks(1:1:size(cell_stability_percentage(:),2))
            if size(cell_stability_percentage(:),1)>1
                xlim([1 16])
            end
        ...
        hold on
        PL2.(sprintf('%s','P',num2str(plot_indx1))) = plot(ratio,'LineWidth',4,'Color',linecolor);
        ...
        if t == 2 && p == 8 % after having plot all the lines
            line([0 16],[0 0],'Color','k','LineStyle','--')
...
for i = 3 : length(files)
    if contains(files(i).name,'4lastLaps.mat')
        ...
        track(1).indices = find(strcmp({cellPopulation_LAPScorr.comparison_type},'between_exposures_REexp-ends_laps') &  strcmp({cellPopulation_LAPScorr.tracks_compared},'[1 3]')==1); % Track 1 & re-exposure
        track(2).indices = find(strcmp({cellPopulation_LAPScorr.comparison_type},'between_exposures_REexp-ends_laps') &  strcmp({cellPopulation_LAPScorr.tracks_compared},'[2 4]')==1); % Track 2 & re-exposure
...
if strcmp(save_option,'Y')
    cd 'X:\BendorLab\Drobo\Lab Members\Marta\Analysis\HIPP\Chapter 1\plField_LAPScorr'
    save(strcat('plotting_data_second_to_first_exposure_',parameter),'session_data')
end
