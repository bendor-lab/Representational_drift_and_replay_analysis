...
function plot_decoding_error_between_exposures(save_option)
...
%     if t == 2
%         rat_numbers = size(betweenExposures_DecodingError,2)-1;
%     else
%         rat_numbers = size(betweenExposures_DecodingError,2);
%     end
%     
%     for r = 1:rat_numbers
%         mat31= [mat2 prot(t).comp(2).template(r,1)];
%         mat13= [mat1 prot(t).comp(1).template(r,1)];
%         
%         mat42= [mat2 prot(t).comp(4).template(r,1)];
%         mat24= [mat1 prot(t).comp(3).template(r,1)];
%         
%         
%         mat13= [mat1 prot(t).comp(1).template(r,1)];
%         mat31= [mat2 prot(t).comp(2).template(r,1)];
...
%%%%%% FIGURE - MEDIAN DECODING ERROR FOR EACH COMPARISON 
    ...
    for t = 1 : size(prot,2) % for each protocol
            ...
            groupsT1 = [groupsT1;  repmat({'RUN1 T1'},size(prot(t).comp(1).template,1),1);repmat({'RUNl T1 Ctrl'},size(prot(t).comp(9).template,1),1)];
            T3_matrix = [T3_matrix; prot(t).comp(2).template(:,1); prot(t).comp(10).template(:,1)]; %3-1 / 3-2
            groupsT3 = [groupsT3;  repmat({'RUN2 T1'},size(prot(t).comp(2).template,1),1); repmat({'RUN2 T2 Ctrl'},size(prot(t).comp(10).template,1),1)];
            T2_matrix = [T2_matrix; prot(t).comp(3).template(:,1); prot(t).comp(11).template(:,1)]; %2-4 / 2-3 (Before 2-1)
            groupsT2 =  [groupsT2; repmat({sprintf('RUN1 T2 (%s)',string(PP.titles.protocols(t)))},size(prot(t).comp(3).template,1),1); repmat({sprintf('RUN1 T2 (%s) Ctrl',string(PP.titles.protocols(t)))},size(prot(t).comp(11).template,1),1)];
            T4_matrix = [T4_matrix; prot(t).comp(4).template(:,1); prot(t).comp(12).template(:,1)]; %4-2 / 4-1 (before 4-3)
            groupsT4 =  [groupsT4; repmat({sprintf('RUN2 T2 (%s)',string(PP.titles.protocols(t)))},size(prot(t).comp(4).template,1),1); repmat({sprintf('RUN2 T2 (%s) Ctrl',string(PP.titles.protocols(t)))},size(prot(t).comp(12).template,1),1)];
    end
    
    T1_matrix([9,13]) = [];% Remove M-BLU-Day2 16X4
    groupsT1([9 13]) = [];
    T3_matrix([9 13]) = [];
    groupsT3([9 13]) = [];
    T2_matrix([9 13]) = [];
    ...
    %     boxplot(first_exposure,'PlotStyle','traditional','Colors',col,'labels',groups1,...
    %         'LabelOrientation','horizontal','Widths',0.5);%,'BoxStyle','filled'
    %
    boxplot(first_exposure,groups1,'PlotStyle','traditional','Colors',col,'LabelOrientation','horizontal','Widths',0.5,'symbol','');
    ...
    for i = 1:length(first_exposure)
        for g = 1:length(all_groups)
            if strcmp(groups1{i}, all_groups{g})
                ii = g;
            end
        end
        h= plot(ii,first_exposure(i),'o','MarkerEdgeColor',col(ii,:),'MarkerFaceColor',col(ii,:));
...
%     set(gca,'XTickLabel',a,'fontsize',16)
    ...
    subplot(2,1,2)
    col = [PP.T1;[0.6 0.6 0.6]; PP.T2(1,:);[0.6 0.6 0.6]; PP.T2(2,:);[0.6 0.6 0.6]; PP.T2(3,:);[0.6 0.6 0.6]; PP.T2(4,:);[0.6 0.6 0.6]; PP.T2(5,:);[0.6 0.6 0.6]]; %set colors
    boxplot(second_exposure,groups2,'PlotStyle','traditional','Colors',col,'LabelOrientation','horizontal','Widths',0.5,'symbol','');
    a = get(get(gca,'children'),'children');   % Get the handles of all the objects
    tt = get(a,'tag');   % List the names of all the objects
    idx = strcmpi(tt,'box');  % Find Box objects
    boxes = a(idx);  % Get the children you need (boxes for first exposure)
    ...
    box off
    hold on
    all_groups = unique(groups2,'stable');
    
    for i = 1:length(second_exposure)
        for g = 1:length(all_groups)
            if strcmp(groups2{i}, all_groups{g})
                ii = g;
            end
        end
        h= plot(ii,second_exposure(i),'o','MarkerEdgeColor',col(ii,:),'MarkerFaceColor',col(ii,:));
        set(h,{'Marker'},{'o'},{'Markersize'},{5},'MarkerFaceColor','w','LineWidth',1.5); %,{'h';'diamond';'o';'square'},{'Markersize'},{6;5;5;6}
