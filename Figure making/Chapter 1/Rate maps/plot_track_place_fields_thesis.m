...
for s = 1 : length(sessions)
    ...
    if s == 1 || s == 6 % plot 16 laps
        ...
        colormap(jet)
        hold on
        ylabel(axes(PL(c)),'Cell ID','FontSize',11);
        yt=place_fields.track(1).sorted_good_cells;
        set(axes(PL(c)),'yticklabel',yt);
        xticks(axes(PL(c)),20:20:100);
        set(axes(PL(c)),'xticklabel',0:40:200);
        if s < 6
            title(axes(PL(c)),[num2str(protocols(s)) ' Laps'],'Position',[0.5 1],'FontSize',11);
        else
            xlabel(axes(PL(c)),'Linearized position (cm)','FontSize',11);
            title(axes(PL(c)),'')
        end
        ...
        if s < 6
            title(axes(PL(c)),'8 Laps','Position',[0.5 1],'FontSize',11);
        else
            xlabel(axes(PL(c)),'Linearized position (cm)','FontSize',11);
            title(axes(PL(c)),'')
        end
